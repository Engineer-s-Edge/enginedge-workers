 ...
 * Redis Cache Adapter
 *
 * Provides Redis-based caching for resume worker.
 */
...
export class RedisCacheAdapter implements OnModuleDestroy {
  ...
  constructor(
    ...
    const redisUrl = this.configService.get<string>('REDIS_URL') || 'redis://localhost:6379/7';
    const db = parseInt(redisUrl.split('/').pop() || '7', 10);
    ...
    this.redis = new Redis({
      ...
      keyPrefix: this.configService.get<string>('REDIS_KEY_PREFIX') || 'resume:',
      lazyConnect: true,
    });

    this.defaultTTL = parseInt(this.configService.get<string>('CACHE_DEFAULT_TTL') || '3600', 10);
    this.keyPrefix = this.configService.get<string>('REDIS_KEY_PREFIX') || 'resume:';
