 ...
 * Redis Cache Adapter
 *
 * Provides Redis-based caching for news articles and API responses.
...
export class RedisCacheAdapter implements OnModuleDestroy {
  ...
  constructor(
    ...
    const db = parseInt(redisUrl.split('/').pop() || '0', 10);
  ...
  async set<T>(key: string, value: T, options?: CacheOptions): Promise<void> {
    try {
      ...
      this.logger.debug('RedisCacheAdapter: Set cache', {
        key: fullKey,
        ttl,
      });
    } catch (error: any) {
