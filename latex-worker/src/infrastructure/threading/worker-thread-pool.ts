...
export class WorkerThreadPool implements OnModuleDestroy {
  ...
  private assignTaskToWorkerWithWorker(worker: Worker): void {
    ...
    const messageHandler = (result: { success: boolean; result?: unknown; error?: string; stack?: string }) => {
      ...
      if (result.success) {
        task.resolve(result.result as typeof task extends WorkerTask<unknown, infer R> ? R : unknown);
      } else {
