...
export class ErrorRecoveryService {
  ...
  parseErrors(log: string): ErrorRecoveryResult {
    ...
    const autoFixSuggestions = canAutoFix
      ? this.generateAutoFixSuggestions(errors)
      : undefined;

    this.logger.log(
      `Parsed ${errorCount} errors and ${warningCount} warnings from compilation log`,
    );
  ...
  generateErrorSummary(result: ErrorRecoveryResult): string {
    ...
    for (const [category, categoryErrors] of byCategory) {
      ...
      for (const error of categoryErrors.slice(0, 3)) {
        // Show max 3 per category
  ...
  private generateAutoFixSuggestions(errors: ParsedError[]): string[] {
    ...
    if (missingPackages.length > 0) {
      suggestions.push(
        `Add to preamble: ${missingPackages.map((p) => `\\usepackage{${p}}`).join(', ')}`,
      );
    }
    ...
    if (undefinedRefs.length > 0) {
      suggestions.push(
        `Run compilation twice to resolve references, or add missing labels: ${undefinedRefs.map((r) => `\\label{${r}}`).join(', ')}`,
      );
    }
