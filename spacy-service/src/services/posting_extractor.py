...
class PostingExtractor:
    ...
    def _detect_sections(self, text: str) -> List[Dict[str, Any]]:
        ...
        section_patterns = [
            ...
            (
                r"(?i)(qualifications|requirements|what we're looking for)",
                "Qualifications",
            ),
    ...
    def _normalize_role_family(self, title: str) -> str:
        ...
        if any(
            kw in title_lower
            for kw in ["software", "engineer", "developer", "programmer"]
        ):
            return "Software Engineer"
        elif any(
            kw in title_lower
            for kw in ["data scientist", "ml engineer", "machine learning"]
        ):
    ...
    def _extract_location(self, doc, text: str) -> Dict[str, Any]:
        ...
        for ent in doc.ents:
            if ent.label_ in ["GPE", "LOC"]:
                locations.append({"city": ent.text, "region": None, "country": None})
    ...
    def _extract_authorization(self, text: str) -> Dict[str, Any]:
        ...
        work_auth_required = any(
            kw in text_lower
            for kw in ["authorized to work", "work authorization", "eligible to work"]
        )
    ...
    def _extract_education(self, doc, text: str) -> Dict[str, Any]:
        ...
        if "bachelor" in text_lower or "bs" in text_lower or "ba" in text_lower:
            requirements.append(
                {"level": "Bachelors", "field": self._extract_degree_fields(text)}
            )

        if "master" in text_lower or "ms" in text_lower or "ma" in text_lower:
            requirements.append(
                {"level": "Masters", "field": self._extract_degree_fields(text)}
            )

        if "phd" in text_lower or "doctorate" in text_lower:
            requirements.append(
                {"level": "PhD", "field": self._extract_degree_fields(text)}
            )
