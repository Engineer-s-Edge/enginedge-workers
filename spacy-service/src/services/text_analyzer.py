...
class TextAnalyzer:
    ...
    def _calculate_metrics(self, text: str, doc) -> Dict[str, Any]:
        ...
        avg_word_length = (
            sum(len(token.text) for token in words) / num_words if num_words > 0 else 0
        )
    ...
    def check_grammar(self, text: str) -> List[Dict[str, Any]]:
        ...
        for i in range(len(tokens) - 1):
            if tokens[i] == tokens[i + 1]:
                issues.append(
                    {"type": "repeated_word", "word": tokens[i], "position": i}
                )
        ...
        for sent in doc.sents:
            ...
            if not has_verb and len(list(sent)) > 3:
                issues.append({"type": "no_verb", "sentence": sent.text})
