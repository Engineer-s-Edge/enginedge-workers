...
def test_strong_bullet_evaluation(evaluator):
    ...
    bullet = "Developed scalable microservices architecture using Node.js and Docker, reducing deployment time by 60%"
...
def test_quantifiable_results(evaluator):
    ...
    # Has metrics
    assert evaluator._check_quantifiable("Improved performance by 50%")['passed'] is True
...
def test_tense_consistency(evaluator):
    ...
    assert evaluator._check_tense("Developed and deployed the application")['passed'] is True
    
    # Consistent present tense
    assert evaluator._check_tense("Develop and deploy applications")['passed'] is True
    
    # Mixed tenses
    assert evaluator._check_tense("Developed and deploy the application")['passed'] is False

def test_keyword_presence(evaluator):
    """Test keyword presence check."""
    # Has relevant keywords
    assert evaluator._check_keywords("Developed Python application using AWS")['passed'] is True
...
def test_fluff_detection(evaluator):
    ...
    # Has fluff
    assert evaluator._check_no_fluff("Leveraged synergies to optimize")['passed'] is False
    assert evaluator._check_no_fluff("Utilized best practices")['passed'] is False

@pytest.mark.parametrize("bullet,expected_min_score", [
