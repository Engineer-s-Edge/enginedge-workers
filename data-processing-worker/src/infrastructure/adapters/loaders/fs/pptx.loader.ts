...
export class PptxLoaderAdapter extends FilesystemLoaderPort {
    ...
    try {
      ...
      const documents = langchainDocs.map((doc: any, index: number) => {
        return new Document(
          ...
          doc.pageContent,
          { source: fileName, sourceType: 'file', mimeType: 'application/vnd.openxmlformats-officedocument.presentationml.presentation', fileName, fileExtension: '.pptx', ...metadata, ...doc.metadata } as DocumentMetadata,
        );
      });
      this.logger.log(`Loaded ${documents.length} slides from PPTX`);
      return documents;
    } catch (error) {
      this.logger.error(`Error loading PPTX: ${error instanceof Error ? error.message : 'Unknown error'}`);
  ...
  supports(source: string | Blob): boolean {
    if (source instanceof Blob) return source.type === 'application/vnd.openxmlformats-officedocument.presentationml.presentation';
