    ...
    try {
      ...
      const documents = langchainDocs.map((doc: any, index: number) => {
        return new Document(
          ...
          doc.pageContent,
          { source: fileName, sourceType: 'file', mimeType: 'application/epub+zip', fileName, fileExtension: '.epub', ...metadata, ...doc.metadata } as DocumentMetadata,
        );
      });
      this.logger.log(`Loaded ${documents.length} documents from EPUB`);
      return documents;
    } catch (error) {
      this.logger.error(`Error loading EPUB: ${error instanceof Error ? error.message : 'Unknown error'}`);
