...
export class GoogleEmbedderAdapter implements EmbedderPort {
  ...
  constructor(
    ...
    if (!key) {
      this.logger.warn(
        'Google API key not provided. Embedder will not be functional.',
      );
    ...
    this.logger.log(
      `Initialized Google embedder: ${this.modelName} (${this.dimensions}d)`,
    );
  }
  ...
  async embedText(text: string): Promise<number[]> {
    if (!this.embeddings) {
      throw new Error(
        'Google embedder not configured. Please set GOOGLE_API_KEY environment variable.',
      );
    }

    this.logger.debug(
      `Generating Google embedding for text (${text.length} chars)`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Error generating Google embedding: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      throw new Error(
        `Failed to generate Google embedding: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  async embedBatch(texts: string[]): Promise<number[][]> {
    ...
    try {
      ...
      this.logger.error(
        `Error generating Google batch embeddings: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      throw new Error(
        `Failed to generate Google batch embeddings: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
