...
export class OpenAIEmbedderAdapter implements EmbedderPort {
  ...
  constructor(
    ...
    if (!key) {
      this.logger.warn('OpenAI API key not provided. Embedder will not be functional.');
    ...
    this.logger.log(`Initialized OpenAI embedder: ${this.modelName} (${this.dimensions}d)`);
  }

  async embedText(text: string): Promise<number[]> {
    if (!this.embeddings) {
      throw new Error('OpenAI embedder not configured. Please set OPENAI_API_KEY environment variable.');
    }
    ...
    try {
      ...
      this.logger.error(`Error generating embedding: ${error instanceof Error ? error.message : 'Unknown error'}`);
  ...
  async embedBatch(texts: string[]): Promise<number[][]> {
    ...
    try {
      ...
      this.logger.error(`Error generating batch embeddings: ${error instanceof Error ? error.message : 'Unknown error'}`);
