...
export class RedisCacheAdapter implements OnModuleDestroy {
  ...
  constructor(
    ...
    const redisUrl = this.configService.get<string>('REDIS_URL') || 'redis://localhost:6379/4';
    ...
    this.redis = new Redis({
      ...
      keyPrefix: this.configService.get<string>('REDIS_KEY_PREFIX') || 'data-processing:',
      lazyConnect: true,
    });

    this.defaultTTL = parseInt(this.configService.get<string>('CACHE_DEFAULT_TTL') || '3600', 10);
    this.keyPrefix = this.configService.get<string>('REDIS_KEY_PREFIX') || 'data-processing:';
  ...
  async deletePattern(pattern: string, namespace?: string): Promise<number> {
    try {
      ...
      this.logger.debug('RedisCacheAdapter: Deleted pattern', { pattern: fullPattern, deletedCount });
      return deletedCount;
    } catch (error: any) {
