...
describe('DocxLoaderAdapter (Phase 1 - DOCX Loader)', () => {
  ...
  it('docx-003: loadBlob can be mocked and returns expected docs', async () => {
    const docs = [new Document('d2', 'docx content', { source: 'file.docx', sourceType: 'file' })];
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob({ size: 10 } as any, 'file.docx', {});
  ...
  it('docx-004: extracts paragraphs from DOCX documents', async () => {
    const docs = [
      new Document('para1', 'First paragraph content', { source: 'file.docx', sourceType: 'file', paragraphIndex: 0 }),
      new Document('para2', 'Second paragraph content', { source: 'file.docx', sourceType: 'file', paragraphIndex: 1 }),
      new Document('para3', 'Third paragraph content', { source: 'file.docx', sourceType: 'file', paragraphIndex: 2 }),
    ];
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob({ size: 5000 } as any, 'file.docx', {});
  ...
  it('docx-005: extracts and preserves table content from DOCX', async () => {
    const docs = [
      new Document('table1', 'Row1-Col1|Row1-Col2\nRow2-Col1|Row2-Col2', { source: 'file.docx', sourceType: 'file', hasTable: true }),
    ];
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob({ size: 2000 } as any, 'file.docx', {});
  ...
  it('docx-006: preserves document properties and formatting', async () => {
    const docs = [new Document('doc', 'content', { source: 'file.docx', sourceType: 'file', author: 'John Doe', createdDate: new Date('2025-01-01') })];
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob({ size: 3000 } as any, 'file.docx', {});
  ...
  it('docx-007: handles documents with multiple sections', async () => {
    const docs = Array.from({ length: 8 }, (_, i) =>
      new Document(`section${i + 1}`, `Content section ${i + 1}`, { source: 'file.docx', sourceType: 'file', section: i + 1 })
    );
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob({ size: 10000 } as any, 'file.docx', {});
  ...
  it('docx-008: handles empty or minimal DOCX files', async () => {
    ...
    const res = await (adapter as any).loadBlob({ size: 500 } as any, 'empty.docx', {});
  ...
  it('docx-010: handles corrupted or malformed DOCX files', async () => {
    jest.spyOn(adapter as any, 'loadBlob').mockRejectedValue(new Error('DOCX parse error'));
    await expect((adapter as any).loadBlob({ size: 1000 } as any, 'corrupted.docx', {})).rejects.toThrow('DOCX parse error');
  });
});
