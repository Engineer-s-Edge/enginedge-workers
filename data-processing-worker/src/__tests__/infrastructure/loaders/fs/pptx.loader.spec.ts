...
describe('PptxLoaderAdapter (Phase 1 - PPTX Loader)', () => {
  ...
  it('pptx-004: extracts slides as separate documents', async () => {
    const docs = [
      new Document('slide1', 'Slide 1: Title and intro', {
        source: 'slides.pptx',
        sourceType: 'file',
        slideNumber: 1,
      }),
      new Document('slide2', 'Slide 2: Content section', {
        source: 'slides.pptx',
        sourceType: 'file',
        slideNumber: 2,
      }),
      new Document('slide3', 'Slide 3: Conclusion', {
        source: 'slides.pptx',
        sourceType: 'file',
        slideNumber: 3,
      }),
  ...
  it('pptx-005: preserves slide titles and speaker notes', async () => {
    const docs = [
      new Document('s1', 'Slide content', {
        source: 'slides.pptx',
        sourceType: 'file',
        title: 'Presentation Title',
        speakerNotes: 'Important notes here',
      }),
    ];
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob(
  ...
  it('pptx-006: handles large presentations with many slides', async () => {
    const docs = Array.from(
      { length: 50 },
      (_, i) =>
        new Document(`slide${i + 1}`, `Slide ${i + 1} content`, {
          source: 'large.pptx',
          sourceType: 'file',
          slideNumber: i + 1,
        }),
  ...
  it('pptx-007: extracts text from shapes and textboxes', async () => {
    const docs = [
      new Document(
        's1',
        'Title text from shape\nContent text from textbox\nBullet points content',
        { source: 'slides.pptx', sourceType: 'file' },
      ),
    ];
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob(
  ...
  it('pptx-010: handles corrupted PPTX gracefully', async () => {
    jest
      ...
      .mockRejectedValue(new Error('PPTX parse error'));
    await expect(
      (adapter as any).loadBlob({ size: 1000 } as any, 'corrupted.pptx', {}),
    ).rejects.toThrow('PPTX parse error');
  });
});
