...
describe('EpubLoaderAdapter (Phase 1 - EPUB Loader)', () => {
  ...
  it('epub-004: extracts chapters from EPUB', async () => {
    const docs = [
      new Document('ch1', 'Chapter 1 content', {
        source: 'book.epub',
        sourceType: 'file',
        chapterIndex: 0,
      }),
      new Document('ch2', 'Chapter 2 content', {
        source: 'book.epub',
        sourceType: 'file',
        chapterIndex: 1,
      }),
      new Document('ch3', 'Chapter 3 content', {
        source: 'book.epub',
        sourceType: 'file',
        chapterIndex: 2,
      }),
  ...
  it('epub-006: handles large EPUB books', async () => {
    const docs = Array.from(
      { length: 25 },
      (_, i) =>
        new Document(`ch${i + 1}`, `Chapter ${i + 1} text`, {
          source: 'large.epub',
          sourceType: 'file',
          chapterIndex: i,
        }),
  ...
  it('epub-007: handles EPUB with TOC (table of contents)', async () => {
    const docs = [
      new Document('ch1', 'content', {
        source: 'book.epub',
        sourceType: 'file',
        toc: 'Introduction,Chapter 1,Chapter 2',
      }),
    ];
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob(
  ...
  it('epub-010: handles corrupted EPUB gracefully', async () => {
    jest
      ...
      .mockRejectedValue(new Error('EPUB parse error'));
    await expect(
      (adapter as any).loadBlob({ size: 1000 } as any, 'corrupted.epub', {}),
    ).rejects.toThrow('EPUB parse error');
  });
});
