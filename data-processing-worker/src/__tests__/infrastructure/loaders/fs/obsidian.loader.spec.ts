...
describe('ObsidianLoaderAdapter (Phase 1 - Obsidian Loader)', () => {
  ...
  it('obsidian-004: preserves wiki-style links', async () => {
    const docs = [
      new Document('n1', 'Text with [[linked note]] and reference', {
        source: 'note.md',
        sourceType: 'file',
      }),
    ];
  ...
  it('obsidian-007: preserves code blocks and callouts', async () => {
    const docs = [
      new Document(
        ...
        { source: 'note.md', sourceType: 'file' },
      ),
    ];
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob(
  ...
  it('obsidian-010: handles malformed vault export gracefully', async () => {
    jest
      ...
      .mockRejectedValue(new Error('Obsidian parse error'));
    await expect(
      (adapter as any).loadBlob({ size: 1000 } as any, 'malformed.md', {}),
    ).rejects.toThrow('Obsidian parse error');
  });
});
