...
describe('SrtLoaderAdapter (Phase 1 - SRT Loader)', () => {
  ...
  it('srt-003: loadBlob returns mocked documents', async () => {
    const docs = [
      new Document('id-srt', 'srt-content', {
        source: 'captions.srt',
        sourceType: 'file',
      }),
    ];
  ...
  it('srt-004: extracts subtitles as separate documents', async () => {
    ...
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await adapter.loadBlob(
      { size: 5000 } as any,
      'captions.srt',
      {},
    );
  ...
  it('srt-006: handles large SRT files with many subtitles', async () => {
    const docs = Array.from(
      { length: 500 },
      (_, i) =>
        new Document(`sub${i + 1}`, `Subtitle ${i + 1}`, {
          source: 'large.srt',
          sourceType: 'file',
          subtitleIndex: i + 1,
        }),
  ...
  it('srt-010: handles malformed SRT gracefully', async () => {
    jest
      ...
      .mockRejectedValue(new Error('SRT parse error'));
    await expect(
      adapter.loadBlob({ size: 1000 } as any, 'malformed.srt', {}),
    ).rejects.toThrow('SRT parse error');
  });
});
