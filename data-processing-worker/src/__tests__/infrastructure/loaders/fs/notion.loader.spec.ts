...
describe('NotionLoaderAdapter (Phase 1 - Notion Loader)', () => {
  ...
  it('notion-002: loadBlob returns mocked documents', async () => {
    const docs = [
      new Document('n1', 'notion', { source: 'page', sourceType: 'file' }),
    ];
  ...
  it('notion-003: extracts pages as separate documents', async () => {
    const docs = Array.from(
      { length: 5 },
      (_, i) =>
        new Document(`page${i + 1}`, `Page ${i + 1} content`, {
          source: `notion.md`,
          sourceType: 'file',
          pageId: `id${i + 1}`,
        }),
  ...
  it('notion-005: handles nested block structures', async () => {
    const docs = [
      new Document(
        'p1',
        'Heading\nParagraph\nList item 1\nList item 2\nToggle content',
        { source: 'complex.md', sourceType: 'file', hasNestedBlocks: true },
      ),
    ];
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob(
  ...
  it('notion-009: handles malformed Notion export gracefully', async () => {
    jest
      ...
      .mockRejectedValue(new Error('Notion parse error'));
    await expect(
      (adapter as any).loadBlob({ size: 1000 } as any, 'malformed.md', {}),
    ).rejects.toThrow('Notion parse error');
  });
  ...
  it('notion-010: preserves code blocks and formatting', async () => {
    const docs = [
      new Document(
        ...
        { source: 'code.md', sourceType: 'file' },
      ),
    ];
    jest.spyOn(adapter as any, 'loadBlob').mockResolvedValue(docs);
    const res = await (adapter as any).loadBlob(
