...
describe('PdfLoaderAdapter (Phase 1 - PDF Loader)', () => {
  ...
  it('pdf-003: can call loadBlob when mocked', async () => {
    const docs = [
      new Document('id-pdf', 'pdf-content', {
        source: 'file.pdf',
        sourceType: 'file',
      }),
    ];
  ...
  it('pdf-004: extracts documents from multi-page PDFs', async () => {
    const docs = [
      new Document('page1', 'page 1 content', {
        source: 'multi.pdf',
        sourceType: 'file',
        pageNumber: 1,
      }),
      new Document('page2', 'page 2 content', {
        source: 'multi.pdf',
        sourceType: 'file',
        pageNumber: 2,
      }),
      new Document('page3', 'page 3 content', {
        source: 'multi.pdf',
        sourceType: 'file',
        pageNumber: 3,
      }),
  ...
  it('pdf-007: handles large PDF files', async () => {
    const largeDocs = Array.from(
      { length: 50 },
      (_, i) =>
        new Document(`page${i + 1}`, `content page ${i + 1}`, {
          source: 'large.pdf',
          sourceType: 'file',
          pageNumber: i + 1,
        }),
  ...
  it('pdf-008: extracts content with special characters and unicode', async () => {
    const docs = [
      new Document('doc1', 'Content with émojis 🚀 and symbols: @#$%', {
        source: 'special.pdf',
        sourceType: 'file',
      }),
    ];
  ...
  it('pdf-009: handles corrupted PDF gracefully', async () => {
    ...
    jest
      ...
      .mockRejectedValue(new Error('PDF corruption'));

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(
      (adapter as any).loadBlob({ size: 1000 } as any, 'corrupted.pdf', {}),
    ).rejects.toThrow('PDF corruption');
  });

  // Test 10: Document ID generation
  it('pdf-010: generates unique document IDs for each page', async () => {
    const docs = [
      new Document('id-page1', 'page 1', {
        source: 'file.pdf',
        sourceType: 'file',
      }),
      new Document('id-page2', 'page 2', {
        source: 'file.pdf',
        sourceType: 'file',
      }),
    ];
