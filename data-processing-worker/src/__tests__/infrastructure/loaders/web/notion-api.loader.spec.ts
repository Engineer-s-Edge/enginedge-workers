...
describe('NotionApiLoaderAdapter (Phase 2 - Notion API Loader)', () => {
  ...
  it('notion-api-002: load returns documents from Notion', async () => {
    const docs = [
      new Document('d-notion-api', 'notion-api-content', {
        source: 'notion://page',
        sourceType: 'url',
      }),
    ];
  ...
  it('notion-api-004: handles nested block structures', async () => {
    const docs = [
      ...
      new Document('d2', 'bullet 1', {
        source: 'notion',
        sourceType: 'url',
        blockType: 'bulleted_list_item',
      }),
      new Document('d3', 'nested bullet', {
        source: 'notion',
        sourceType: 'url',
        blockType: 'bulleted_list_item',
        indent: 1,
  ...
  it('notion-api-005: handles database query results', async () => {
    const docs = Array.from(
      { length: 50 },
      (_, i) =>
        new Document(`d${i}`, `row${i}`, {
          source: 'notion',
          sourceType: 'url',
          blockType: 'database_row',
        }),
    );
    jest.spyOn(adapter as any, 'load').mockResolvedValue(docs);
  ...
  it('notion-api-006: preserves rich text formatting', async () => {
    const docs = [
      new Document('d1', 'formatted text', {
        source: 'notion',
        sourceType: 'url',
        hasCode: true,
        isBold: true,
  ...
  it('notion-api-007: handles API rate limiting', async () => {
    ...
    await expect(
      (adapter as any).load('https://notion.so/page'),
