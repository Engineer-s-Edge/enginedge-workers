...
describe('HtmlLoaderAdapter (Phase 2 - HTML Loader)', () => {
  ...
  it('html-002: load returns documents from HTML', async () => {
    const docs = [
      new Document('d-html', '<p>content</p>', {
        source: 'html://content',
        sourceType: 'url',
      }),
    ];
  ...
  it('html-003: extracts text from HTML structure', async () => {
    ...
    const res = await (adapter as any).load(
      '<html><h1>Heading</h1><p>Paragraph content</p></html>',
    );
  ...
  it('html-004: handles nested HTML elements', async () => {
    ...
    jest.spyOn(adapter as any, 'load').mockResolvedValue(docs);
    const res = await (adapter as any).load(
      '<html><ul><li><a>link</a></li></ul></html>',
    );
  ...
  it('html-005: preserves link information', async () => {
    const docs = [
      new Document('d1', 'text with links', {
        source: 'html',
        sourceType: 'url',
        links: [
  ...
  it('html-007: handles malformed HTML gracefully', async () => {
    jest
      ...
      .mockRejectedValue(new Error('Invalid HTML structure'));
    await expect(
      (adapter as any).load('<html><body><unclosed>'),
  ...
  it('html-008: handles empty or minimal HTML', async () => {
    jest
      .spyOn(adapter as any, 'load')
      .mockRejectedValue(new Error('No content in HTML'));
    await expect((adapter as any).load('<html></html>')).rejects.toThrow(
      'content',
    );
  });
});
