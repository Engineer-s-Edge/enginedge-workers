...
describe('YoutubeLoaderAdapter (Phase 2 - YouTube Loader)', () => {
  ...
  it('youtube-002: load returns transcript documents', async () => {
    const docs = [
      new Document('d-yt', 'yt-transcript', {
  ...
  it('youtube-004: handles multiple language transcripts', async () => {
    const docs = [
      new Document('d1', 'english text', {
        source: 'youtube',
        sourceType: 'url',
        language: 'en',
      }),
      new Document('d2', 'spanish text', {
        source: 'youtube',
        sourceType: 'url',
        language: 'es',
      }),
      new Document('d3', 'french text', {
        source: 'youtube',
        sourceType: 'url',
        language: 'fr',
      }),
  ...
  it('youtube-005: preserves transcript timestamps', async () => {
    const docs = [
      new Document('d1', '0:00 intro 1:30 main content', {
        source: 'youtube',
        sourceType: 'url',
        hasTimestamps: true,
        captioned: true,
  ...
  it('youtube-006: handles long videos', async () => {
    const docs = [
      new Document('d1', 'transcript content'.repeat(100), {
        source: 'youtube',
        sourceType: 'url',
        duration: 36000,
      }),
  ...
  it('youtube-007: handles videos without transcripts', async () => {
    ...
    await expect(
      (adapter as any).load('https://youtube.com/watch?v=notranscript'),
  ...
  it('youtube-008: handles invalid video IDs and private videos', async () => {
    jest
      .spyOn(adapter as any, 'load')
      .mockRejectedValue(new Error('Video not found or private'));
    await expect(
      (adapter as any).load('https://youtube.com/watch?v=invalid'),
