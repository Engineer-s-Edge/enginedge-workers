...
describe('PlaywrightWebLoaderAdapter (Phase 2 - Playwright Web Loader)', () => {
  ...
  it('playwright-003: waits for JavaScript execution', async () => {
    const docs = [
      new Document('d1', 'rendered content', {
        source: 'https://spa.com',
        sourceType: 'url',
        rendered: true,
      }),
    ];
  ...
  it('playwright-004: handles page interactions', async () => {
    const docs = [
      new Document('d1', 'expanded content', {
  ...
  it('playwright-005: captures JavaScript-rendered content', async () => {
    const docs = [
      new Document('d1', 'content after JS', {
  ...
  it('playwright-007: handles timeout during page load', async () => {
    jest
      ...
      .mockRejectedValue(new Error('Page load timeout'));
    await expect((adapter as any).load('https://timeout.com')).rejects.toThrow(
      'timeout',
    );
  });
  ...
  it('playwright-008: handles navigation errors', async () => {
    jest
      .spyOn(adapter as any, 'load')
      .mockRejectedValue(new Error('Page navigation failed'));
    await expect((adapter as any).load('https://error.com')).rejects.toThrow(
      'navigation',
    );
  });
});
