...
describe('RecursiveUrlLoaderAdapter (Phase 2 - Recursive URL Loader)', () => {
  ...
  it('recursive-url-002: load returns documents', async () => {
    const docs = [new Document('d-rec', 'rec-content', { source: 'https://root', sourceType: 'url' })];
  ...
  it('recursive-url-005: respects crawl depth limits', async () => {
    ...
    jest.spyOn(adapter as any, 'load').mockResolvedValue(docs);
    const res = await (adapter as any).load('https://example.com');
    const maxDepth = Math.max(...res.map((d: any) => d.metadata.depth as number));
    expect(maxDepth).toBeLessThanOrEqual(3);
  });

  it('recursive-url-005: extracts and follows internal links', async () => {
    const docs = [
      new Document('d1', 'home', { source: 'https://example.com/', sourceType: 'url', links: ['about', 'contact'] }),
      new Document('d2', 'about', { source: 'https://example.com/about', sourceType: 'url', links: [] }),
      new Document('d3', 'contact', { source: 'https://example.com/contact', sourceType: 'url', links: [] })
    ];
    jest.spyOn(adapter as any, 'load').mockResolvedValue(docs);
    const res = await (adapter as any).load('https://example.com');
  ...
  it('recursive-url-006: avoids duplicate URLs', async () => {
    const docs = [
      new Document('d1', 'page', { source: 'https://example.com/page', sourceType: 'url' })
    ];
    jest.spyOn(adapter as any, 'load').mockResolvedValue(docs);
    const res = await (adapter as any).load('https://example.com');
  ...
  it('recursive-url-007: handles robots.txt and crawl restrictions', async () => {
    jest.spyOn(adapter as any, 'load').mockRejectedValue(new Error('Crawling disallowed by robots.txt'));
    await expect((adapter as any).load('https://restricted.com')).rejects.toThrow('robots.txt');
  });

  it('recursive-url-008: handles crawl timeout', async () => {
    jest.spyOn(adapter as any, 'load').mockRejectedValue(new Error('Crawl operation timeout'));
    await expect((adapter as any).load('https://slow.com')).rejects.toThrow('timeout');
  });
});
