...
describe('CurlWebLoaderAdapter (Phase 2 - Curl Web Loader)', () => {
  ...
  it('curl-002: load returns documents', async () => {
    const docs = [
      new Document('d-curl', 'curl-content', {
  ...
  it('curl-004: retries on failure', async () => {
    jest
      ...
      .mockResolvedValueOnce(new Error('timeout'))
  ...
  it('curl-005: handles redirects', async () => {
    const docs = [
      new Document('d1', 'final content', {
        source: 'https://final.example.com',
        sourceType: 'url',
        redirectFrom: 'https://redirect.example.com',
      }),
    ];
    jest.spyOn(adapter as any, 'load').mockResolvedValue(docs);
  ...
  it('curl-006: handles large response bodies', async () => {
    const docs = [
      new Document('d1', 'x'.repeat(10000), {
        source: 'https://example.com',
        sourceType: 'url',
      }),
    ];
  ...
  it('curl-007: handles timeout errors gracefully', async () => {
    ...
    await expect(
      (adapter as any).load('https://slow.example.com'),
  ...
  it('curl-008: handles HTTP error codes', async () => {
    ...
    await expect(
      (adapter as any).load('https://notfound.example.com'),
    ).rejects.toThrow('404');
  });
});
