...
describe('DocumentController', () => {
  ...
  beforeEach(() => {
    ...
    docService.processDocument.mockResolvedValue({
      documentIds: ['d1'],
      chunks: 1,
    });
    docService.searchSimilar.mockResolvedValue([]);
    docService.getDocument.mockResolvedValue(
      new Document('d1', 'hello', { source: 's', sourceType: 'file' }),
    );
  ...
  it('processUrl calls service and returns success', async () => {
    const res = await controller.processUrl(
      'http://example.com',
      true,
      true,
      true,
