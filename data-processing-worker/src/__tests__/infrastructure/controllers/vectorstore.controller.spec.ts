...
describe('VectorStoreController (Phase 7 Integration)', () => {
  ...
  describe('search endpoint', () => {
    it('001: searches with valid embedding and returns results', async () => {
      const results = [
        {
          document: new Document('d1', 'content', {
            source: 's',
            sourceType: 'file',
          }),
          score: 0.95,
        ...
        {
          document: new Document('d2', 'other', {
            source: 's',
            sourceType: 'file',
          }),
          score: 0.85,
  ...
  describe('hybrid-search endpoint', () => {
    it('006: hybrid search combines text and vector results', async () => {
      const results = [
        {
          document: new Document('d1', 'hybrid', {
            source: 's',
            sourceType: 'file',
          }),
          score: 0.92,
  ...
  describe('getDocument endpoint', () => {
    it('011: gets document by ID', async () => {
      const doc = new Document('d1', 'content', {
        source: 's',
        sourceType: 'file',
      });
    ...
    it('013: getDocument with multiple IDs in sequence', async () => {
      const doc1 = new Document('d1', 'c1', {
        source: 's',
        sourceType: 'file',
      });
      const doc2 = new Document('d2', 'c2', {
        source: 's',
        sourceType: 'file',
      });
    ...
    it('015: getDocument with metadata', async () => {
      ...
      expect(res.document?.metadata).toEqual({
        source: 'file.pdf',
        sourceType: 'file',
      });
    });
  ...
  describe('updateDocument endpoint', () => {
    it('021: updates document content', async () => {
      ...
      expect(vectorStore.updateDocument).toHaveBeenCalledWith(
        'd1',
  ...
  describe('deleteDocuments endpoint', () => {
    ...
    it('030: delete error handling', async () => {
      vectorStore.deleteDocuments.mockRejectedValue(
        new Error('deletion failed'),
      );

      await expect(controller.deleteDocuments({ ids: ['d1'] })).rejects.toThrow(
        'deletion failed',
      );
    });
  });
