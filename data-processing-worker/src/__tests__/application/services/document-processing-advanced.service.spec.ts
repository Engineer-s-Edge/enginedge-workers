...
describe('DocumentProcessingService Advanced (Phase 3 - LoaderService Extended)', () => {
  ...
  it('adv-proc-001: detects loader type from extension', async () => {
    const pdfLoader: any = { load: async () => [new Document('d1', 'pdf', { source: 'file.pdf', sourceType: 'file' })], supports: () => true, getSupportedTypes: () => ['.pdf'] };
  ...
  it('adv-proc-002: processes document with custom options', async () => {
    ...
    const result = await service.processDocument('file.txt', { loaderName: 'file', split: false, embed: false });
    expect(result.documentIds.length).toBeGreaterThan(0);
  });

  it('adv-proc-003: chains multiple transformations correctly', async () => {
    const loader: any = { load: async () => [new Document('d1', 'text', { source: 'file.txt', sourceType: 'file' })], supports: () => true, getSupportedTypes: () => ['.txt'] };
    loaderRegistry.registerLoader('file', loader);
  ...
  it('adv-proc-004: handles multiple documents from single source', async () => {
    const loader: any = {
      load: async () => [
        new Document('d1', 'doc1', { source: 'archive.zip', sourceType: 'file' }),
        new Document('d2', 'doc2', { source: 'archive.zip', sourceType: 'file' }),
        new Document('d3', 'doc3', { source: 'archive.zip', sourceType: 'file' }),
      ],
      supports: () => true,
    ...
    const result = await service.processDocument('archive.zip', { loaderName: 'archive', split: false });
    expect(result.documentIds.length).toBe(3);
  });

  it('adv-proc-005: respects document metadata throughout processing', async () => {
    const loader: any = {
      load: async () => [new Document('d1', 'content', { 
        source: 'file.txt', 
        sourceType: 'file',
        author: 'John Doe',
  ...
  it('adv-proc-006: handles URL-based loaders', async () => {
    const webLoader: any = {
      load: async (url: string) => [new Document('d-web', 'html content', { source: url, sourceType: 'url' })],
      supports: (source: string) => source.startsWith('http'),
      getSupportedTypes: () => [],
    };
    loaderRegistry.registerLoader('web', webLoader);
    
    const result = await service.processDocument('https://example.com', { loaderName: 'web', split: false });
  ...
  it('adv-proc-007: processes streaming without storing', async () => {
    ...
    const result = await service.processDocument('file.txt', { loaderName: 'file', split: false, embed: true, store: false });
    expect(result.documentIds).toBeDefined();
  });

  it('adv-proc-008: validates loader availability before processing', async () => {
    await expect(service.processDocument('file.unknown', { loaderName: 'nonexistent' })).rejects.toThrow();
  });

  it('adv-proc-009: batch processes multiple files with same loader', async () => {
    const loader: any = {
      load: async (source: string) => [new Document(`d-${source}`, `content-${source}`, { source, sourceType: 'file' })],
      supports: () => true,
      getSupportedTypes: () => ['.txt'],
    };
    loaderRegistry.registerLoader('file', loader);
    
    const result1 = await service.processDocument('file1.txt', { loaderName: 'file', split: false });
    const result2 = await service.processDocument('file2.txt', { loaderName: 'file', split: false });
  ...
  it('adv-proc-010: recovers from partial processing failures', async () => {
    ...
    await expect(service.processDocument('fail.txt', { loaderName: 'file' })).rejects.toThrow();
    
    // Should succeed for good.txt
    const result = await service.processDocument('good.txt', { loaderName: 'file', split: false });
    expect(result.documentIds).toContain('d1');
  });

  it('adv-proc-011: handles option combinations correctly', async () => {
    const loader: any = { load: async () => [new Document('d1', 'x'.repeat(100), { source: 'file.txt', sourceType: 'file' })], supports: () => true, getSupportedTypes: () => ['.txt'] };
    loaderRegistry.registerLoader('file', loader);
    
    // split=true, embed=false, store=false
    const result1 = await service.processDocument('file.txt', { loaderName: 'file', split: true, embed: false, store: false });
    expect(result1.chunks).toBeDefined();
    
    // split=false, embed=true, store=false
    const result2 = await service.processDocument('file.txt', { loaderName: 'file', split: false, embed: true, store: false });
  ...
  it('adv-proc-012: preserves document order through pipeline', async () => {
    const loader: any = {
      load: async () => [
        ...
        new Document('d2', 'second', { source: 'file.txt', sourceType: 'file' }),
  ...
  it('adv-proc-013: handles empty loader results', async () => {
    const loader: any = { load: async () => [], supports: () => true, getSupportedTypes: () => ['.txt'] };
    loaderRegistry.registerLoader('file', loader);
    
    const result = await service.processDocument('empty.txt', { loaderName: 'file' });
  ...
  it('adv-proc-015: skips storing when not requested', async () => {
    ...
    const result = await service.processDocument('file.txt', { loaderName: 'file', split: false, store: false });
    expect(result.documentIds).toContain('d1');
  });
});
