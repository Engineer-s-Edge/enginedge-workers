...
describe('DocumentProcessingService (Phase 3 - LoaderService)', () => {
  ...
  it('doc-proc-001: processes document with splitting, embedding and storing', async () => {
    const dummyLoader: any = {
      load: async (source: string) => [
        new Document('doc1', 'hello world', { source, sourceType: 'file' }),
      ],
      supports: () => true,
  ...
  it('doc-proc-002: throws when required dependencies are missing for embedding', async () => {
    ...
    const loader: any = {
      load: async () => [
        new Document('d1', 'x', { source: 's', sourceType: 'file' }),
      ],
      supports: () => true,
      getSupportedTypes: () => ['.txt'],
    };
  ...
  it('doc-proc-004: handles multiple loaders gracefully', async () => {
    const loader1: any = {
      load: async () => [
        new Document('d1', 'txt', { source: 'file.txt', sourceType: 'file' }),
      ],
      supports: () => true,
      getSupportedTypes: () => ['.txt'],
    };
    const loader2: any = {
      load: async () => [
        new Document('d2', 'doc', { source: 'file.docx', sourceType: 'file' }),
      ],
      supports: () => true,
    ...
    const result2 = await service.processDocument('file.docx', {
      loaderName: 'docx',
      split: false,
    });
  ...
  it('doc-proc-007: handles loader errors gracefully', async () => {
    ...
    loaderRegistry.registerLoader('file', loader);
    await expect(
      service.processDocument('file.txt', { loaderName: 'file' }),
    ).rejects.toThrow();
  });

  it('doc-proc-008: skips embedding when embedder is undefined', async () => {
    const svc2 = new DocumentProcessingService(
      loaderRegistry,
      undefined as any,
      undefined as any,
      undefined as any,
    ...
    loaderRegistry.registerLoader('file', loader);
    const result = await svc2.processDocument('file.txt', {
      loaderName: 'file',
      split: false,
      embed: false,
    });
  ...
  xit('doc-proc-009: handles large files without splitting', async () => {
    ...
    loaderRegistry.registerLoader('file', loader);
    const result = await service.processDocument('large.txt', {
      loaderName: 'file',
      split: false,
    });
    expect(result.chunks).toBe(1);
  ...
  it('doc-proc-010: handles empty document gracefully', async () => {
    const loader: any = {
      load: async () => [
        new Document('d1', '', { source: 'empty.txt', sourceType: 'file' }),
      ],
      supports: () => true,
      getSupportedTypes: () => ['.txt'],
    };
