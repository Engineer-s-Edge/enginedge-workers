...
export class IdentityService {
  ...
  async login(email: string, password: string) {
    ...
    const accessToken = await this.jwt.issueJwt(
      user._id!,
      user.tenantId,
      tenantSlug,
      roles,
    ...
    const refreshToken = await this.jwt.issueRefresh(
      user._id!,
  ...
  async refresh(refreshToken: string) {
    const { payload } = await jwtVerify(
      refreshToken,
      (this.jwt as any)['privateKey'],
    );
    if (payload.type !== 'refresh' || !payload.jti)
      throw new UnauthorizedException('Invalid refresh');
    if (await this.refreshRepo.isRevoked(String(payload.jti)))
    ...
    const newRefresh = await this.jwt.issueRefresh(
      userId,
      process.env.REFRESH_TOKEN_TTL || '30d',
    );
    return {
