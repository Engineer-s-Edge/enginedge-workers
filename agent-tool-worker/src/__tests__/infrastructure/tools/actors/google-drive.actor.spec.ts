...
import {
  ...
  GoogleDriveArgs,
  GoogleDriveOutput,
} from '@infrastructure/tools/actors/google-drive.actor';
...
describe('GoogleDriveActor', () => {
  ...
  describe('Upload File', () => {
    ...
    it('should upload a file successfully', async () => {
      ...
      expect((result.output as GoogleDriveOutput).operation).toBe(
        'upload-file',
      );
      expect((result.output as GoogleDriveOutput).fileId).toMatch(/^file-\d+$/);
      expect((result.output as GoogleDriveOutput).webViewLink).toContain(
    ...
    it('should return error when fileName is missing', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error?.message).toBe(
        'File name and content are required for file upload',
      );
  ...
  describe('Download File', () => {
    it('should download a file successfully', async () => {
      ...
      expect((result.output as GoogleDriveOutput).fileContent).toBe(
        'SGVsbG8gV29ybGQ=',
      );
    ...
    it('should return error when fileId is missing', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error?.message).toBe(
        'File ID is required for file download',
      );
      expect(result.error?.name).toBe('ValidationError');
    });
  });
  ...
  describe('List Files', () => {
    it('should list files successfully', async () => {
      ...
      expect(Array.isArray((result.output as GoogleDriveOutput).files)).toBe(
        true,
      );
      expect(
        (result.output as GoogleDriveOutput).files!.length,
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe('Delete File', () => {
    ...
    it('should return error when fileId is missing', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error?.message).toBe(
        'File ID is required for file deletion',
      );
      expect(result.error?.name).toBe('ValidationError');
    });
  });
  ...
  describe('Create Folder', () => {
    it('should create a folder successfully', async () => {
      ...
      expect((result.output as GoogleDriveOutput).fileId).toMatch(
        /^folder-\d+$/,
      );
      expect((result.output as GoogleDriveOutput).webViewLink).toContain(
    ...
    it('should return error when folderName is missing', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error?.message).toBe(
        'Folder name is required for folder creation',
      );
      expect(result.error?.name).toBe('ValidationError');
    });
  });
  ...
  describe('Get File Metadata', () => {
    it('should get file metadata successfully', async () => {
      ...
      const metadata = (result.output as GoogleDriveOutput).metadata as {
        id: string;
        name: string;
      };
    ...
    it('should return error when fileId is missing', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error?.message).toBe(
        'File ID is required for metadata retrieval',
      );
      expect(result.error?.name).toBe('ValidationError');
    });
  });
