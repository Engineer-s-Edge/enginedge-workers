...
export class RedisCacheAdapter implements OnModuleDestroy {
  ...
  constructor(
    ...
    const redisUrl = this.configService.get<string>('REDIS_URL') || 'redis://localhost:6379/3';
    ...
    this.redis = new Redis({
      ...
      keyPrefix: this.configService.get<string>('REDIS_KEY_PREFIX') || 'agent-tool:',
      lazyConnect: true,
    });

    this.defaultTTL = parseInt(this.configService.get<string>('CACHE_DEFAULT_TTL') || '3600', 10);
    this.keyPrefix = this.configService.get<string>('REDIS_KEY_PREFIX') || 'agent-tool:';
