...
> {
  ...
  constructor() {
    const errorEvents = [
      new ErrorEvent(
        'HttpRequestError',
        'HTTP request failed - check URL, method, and network connectivity',
        false,
      ),
      new ErrorEvent(
        'SecurityError',
        'Request blocked for security reasons - check URL allowlist',
        false,
      ),
      new ErrorEvent(
        'TimeoutError',
        'HTTP request timed out - consider increasing timeout or checking network',
        true,
      ),
      new ErrorEvent(
        'RateLimitError',
  ...
  private validateRequest(args: HttpRequestArgs): void {
    ...
    if (!['http:', 'https:'].includes(url.protocol)) {
      throw Object.assign(
        new Error('Only HTTP and HTTPS protocols are allowed'),
        {
          name: 'SecurityError',
        },
      );
    ...
    if (blockedHosts.includes(hostname)) {
      throw Object.assign(
        new Error(`Access to ${hostname} is blocked for security reasons`),
        {
          name: 'SecurityError',
        },
      );
    ...
    const privateRanges = [
      { prefix: '10.', description: 'private network (10.0.0.0/8)' },
      {
        ...
        check: (h: string) =>
          h.startsWith('172.') &&
          parseInt(h.split('.')[1]) >= 16 &&
          parseInt(h.split('.')[1]) <= 31,
        description: 'private network (172.16.0.0/12)',
      },
      { prefix: '192.168.', description: 'private network (192.168.0.0/16)' },
      {
        prefix: '169.254.',
        description: 'link-local network (169.254.0.0/16)',
      },
    ];

    for (const range of privateRanges) {
      if (
        range.check ? range.check(hostname) : hostname.startsWith(range.prefix)
      ) {
        throw Object.assign(
          new Error(`Access to ${hostname} is blocked for security reasons`),
          {
