...
> {
  ...
  readonly description =
    'Query Wolfram Alpha computational knowledge engine via Wolfram worker';
  ...
  constructor() {
    const errorEvents = [
      new ErrorEvent(
        ...
        'Wolfram Alpha query failed - check query syntax and try again',
        false,
      ),
      new ErrorEvent(
        'wolfram-timeout',
        'Wolfram Alpha request timed out - consider simplifying the query',
        true,
      ),
      new ErrorEvent(
        'wolfram-service-unavailable',
        'Wolfram Alpha service is unavailable - check service status',
        false,
      ),
    ];
  ...
  protected async retrieve(
    ...
    if (!query || typeof query !== 'string' || query.trim().length === 0) {
      throw Object.assign(
        new Error('Query parameter is required and must be a non-empty string'),
        {
    ...
    if (!validFormats.includes(format)) {
      throw Object.assign(
        new Error(`Invalid format. Must be one of: ${validFormats.join(', ')}`),
        {
          name: 'ValidationError',
        },
      );
  ...
  private async sendWolframRequest(request: {
    ...
    try {
      ...
      const response: AxiosResponse = await axios.post(
        'http://enginedge-local-kernel:5001/compute',
        {
          code: request.query,
        },
        {
      ...
      const errorMessage =
        ...
        'Unknown error';
      throw Object.assign(
        new Error(`Wolfram computation failed: ${errorMessage}`),
        {
          name: 'WolframError',
        },
      );
