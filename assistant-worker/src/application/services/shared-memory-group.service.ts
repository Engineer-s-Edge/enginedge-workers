  ...
  async ensureGroup(
    ...
    try {
      ...
      if (!conversation) {
        ...
        // Note: conversationId will be generated by the repository
        const created = await this.conversationsService.createConversation({
          userId: `graph-${graphId}`,
          rootAgentId: `graph-${graphId}`, // Using graphId as rootAgentId
          type: 'graph',
          settingsOverrides: {
            custom: {
              graphId,
      ...
      switch (request.operation) {
        ...
        case 'addMessage': {
          ...
          const message = Message.create(
            request.params.message.role as 'user' | 'assistant' | 'system',
