...
import { IMessagePublisher, QueueMessage } from '@application/ports/message-queue.port';

@Injectable()
export class ProcessLLMRequestUseCase {
  constructor(
    @Inject('ILLMService') private readonly llmService: ILLMService,
    @Inject('IMessagePublisher') private readonly messagePublisher: IMessagePublisher
  ) {}
  ...
  async execute(request: LLMRequest): Promise<LLMResponse> {
    try {
      ...
      return LLMResponse.error(request.id, error instanceof Error ? error.message : 'Unknown error');
    }
  }
}