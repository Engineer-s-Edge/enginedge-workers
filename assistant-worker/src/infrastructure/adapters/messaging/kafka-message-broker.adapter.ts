...
export class KafkaMessageBrokerAdapter implements MessageBrokerPort, OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    this.retryDelayMs = this.configService.get<number>('KAFKA_RETRY_DELAY_MS', 1000);
  ...
  private async sendToDLQ(
    ...
    try {
      ...
      this.logger.warn(`Message sent to DLQ topic ${dlqTopic} after ${retryCount} retries`, {
        originalTopic: topic,
  ...
  private async processMessageWithRetry(
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        this.metrics.recordKafkaMessageProcessingDuration(topic, durationSeconds);
        
        // Success - log if retried
        if (attempt > 0) {
          this.logger.log(`Message processed successfully after ${attempt} retries`, {
            topic,
            attempt,
            durationSeconds,
          });
        ...
        if (attempt < this.maxRetries) {
          ...
          this.logger.warn(`Message processing failed, retrying (${attempt + 1}/${this.maxRetries})`, {
            topic,
            attempt: attempt + 1,
            delayMs,
          ...
          this.logger.error(`Message processing failed after ${this.maxRetries} retries, sending to DLQ`, {
            topic,
            error: lastError.message,
            durationSeconds,
          });
  ...
  async subscribe(topic: string, handler: (message: unknown) => Promise<void>): Promise<void> {
    try {
      ...
      this.logger.log(`Subscribed to topic: ${topic} (max retries: ${this.maxRetries})`);
      ...
      await this.consumer.run({
        eachMessage: async ({ topic: receivedTopic, partition, message }) => {
          ...
          try {
            ...
            await this.processMessageWithRetry(receivedTopic, parsedMessage, handler);
          } catch (parseError) {
