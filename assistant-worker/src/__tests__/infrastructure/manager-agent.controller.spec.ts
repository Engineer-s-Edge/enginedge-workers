...
describe('ManagerAgentController', () => {
  ...
  const mockManagerInstance = Object.assign(
    Object.create(mockManagerAgentPrototype),
    {
      id: mockAgentId,
      state: {
        status: 'idle',
  ...
  describe('decomposeTask', () => {
    it('should decompose task using runtime service', async () => {
      (agentService.getAgentInstance as jest.Mock).mockResolvedValue(
        mockManagerInstance,
      );
  ...
  describe('getManagerState', () => {
    ...
    it('should throw BadRequest if agent is not a manager', async () => {
      ...
      await expect(
        controller.getManagerState(mockAgentId, mockUserId),
  ...
  describe('reassignTask', () => {
    ...
    it('should reassign task successfully', async () => {
      ...
      (managerRuntime.reassignAssignment as jest.Mock).mockReturnValue(
        undefined,
      );
    ...
    it('should throw BadRequest if assignmentId is missing', async () => {
      ...
      await expect(
        controller.reassignTask(mockAgentId, invalidBody as any),
      ).rejects.toThrow(BadRequestException);
    });
  });
});
