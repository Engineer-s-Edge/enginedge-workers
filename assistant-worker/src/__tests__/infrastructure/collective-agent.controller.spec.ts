...
describe('CollectiveAgentController', () => {
  ...
  const mockInstance = Object.assign(
    Object.create(mockCollectiveAgentPrototype),
    {
  ...
  beforeEach(async () => {
    ...
    controller = module.get<CollectiveAgentController>(
  ...
  describe('orchestrateTask', () => {
    it('should execute orchestrateTask', async () => {
      (executeAgentUseCase.execute as jest.Mock).mockResolvedValue({
        status: 'queued',
      } as any);
  ...
  describe('getSubAgents', () => {
    ...
    it('should throw BadRequest if agent is not collective', async () => {
      ...
      await expect(
        controller.getSubAgents('agent-123', 'user-1'),
